    ...
    # BUG FIX 1: pattern_type from cycles.py is "cycle_length_3/4/5", not "cycle"
    # Map ALL cycle variants + smurfing variants correctly
    RING_RISK_BASE = {
        "cycle_length_3": 95,
        "cycle_length_4": 92,
        "cycle_length_5": 90,
    ...
    # BUG FIX 2: account_patterns is passed in AFTER being populated for this ring
    ...
    async def analyze(file: UploadFile = File(...)):
        ...
        # ── Build Graph ──
        # BUG FIX 3: Use MultiDiGraph so multiple transactions between same pair
        # are all preserved (iterrows was silently overwriting duplicate edges)
        G = nx.MultiDiGraph()
        ...
        # Detectors expect DiGraph for degree/cycle analysis — create a simple view
        # (simple DiGraph collapses multi-edges which is fine for structural detection)
        G_simple = nx.DiGraph(G)

        all_accounts = set(G_simple.nodes())
        ...
        for idx, ring in enumerate(deduped_rings):
            ...
            # BUG FIX 2: compute risk score directly from ring data, not from
            # account_patterns (which isn't populated yet at this point)
